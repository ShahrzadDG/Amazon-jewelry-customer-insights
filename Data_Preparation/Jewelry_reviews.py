...
for Review_chunk in df_review:
    ...
    for (y, m), part_df in out.groupby(["year", "month"], sort=False):
        ...
        part_df["parent_asin"] = part_df["parent_asin"].astype("string")
        part_df["brand"] = part_df["brand"].astype("string")
        part_df["brand_name"] = part_df["brand_name"].astype("string")
        part_df["manufacturer"] = part_df["manufacturer"].astype("string")
        part_df["product_title"]= part_df["product_title"].astype("string")
        part_df["review_title"]= part_df["review_title"].astype("string")
        part_df["review"] = part_df["review"].astype("string")
        part_df["asin"] = part_df["asin"].astype("string")
        part_df["average_rating"]= pd.to_numeric(part_df["average_rating"], errors="coerce")
        part_df["rating_number"] = pd.to_numeric(part_df["rating_number"], errors="coerce")
        part_df["price"]= pd.to_numeric(part_df["price"], errors="coerce")
        part_df["rating"] = pd.to_numeric(part_df["rating"], errors="coerce")
